...
def render_monitoring(train_df: pd.DataFrame, model, threshold: float) -> None:
    ...
    high_psi = psi_df[psi_df["severity"] == "high"] if (not psi_df.empty and "severity" in psi_df.columns) else pd.DataFrame()

    if significant.empty and high_psi.empty:
        st.success("No major drift indicators detected.")
        if psi_df.empty:
            st.info("PSI could not be computed because required monitoring columns were missing in the uploaded batch.")
    else:
